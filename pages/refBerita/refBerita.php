<?php
class refBerita extends baseObject{
  var $Prefix = "refBerita";
  var $formName = "refBeritaForm";
  ...
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($refBerita_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($judulBerita)){
          $err = "Isi Judul";
        }elseif(empty($deskripsiBerita)){
          $err = "Isi Deskripsi";
        }
        if(empty($err)){
          $dataInsert = array(
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($refBerita_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$refBerita_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/refBerita/refBerita.js'></script>
  ...
  function setKolomData($no,$arrayData){
    ...
    <tr class='$classRow'>
        ...
        <td style='vertical-align:middle;'>".$this->generateDate($tanggal)."</td>
                          ...
                          <div class='fileinput-preview thumbnail' data-trigger='fileinput' style='width: 200px; height: 150px;'><img id='thumbnailImages'></img></div>
                            <div>
                                <span class='btn btn-default btn-file'>
                                    ...
                                    <input type='hidden' name='gambarBerita' id='gambarBerita' >
                                    <input type='file' id='fileInputBerita' onChange=$this->Prefix.imageChanged(); accept='image/x-png,image/gif,image/jpeg'>
                                </span>
                          ...
                          <div class='fileinput-preview thumbnail' data-trigger='fileinput' style='width: 200px; height: 150px;'><img id='thumbnailImages' src ='".$getDataEdit['gambar']."'></img></div>
                            <div>
                                <span class='btn btn-default btn-file'>
                                    <span class='fileinput-image'>Select image</span>
                                    <span class='fileinput-exists'>Change</span>
                                    <input type='hidden' name='gambarBerita' id='gambarBerita' value='".$this->imageToBase($getDataEdit['gambar'])."'  >
                                    <input type='file' id='fileInputBerita' onChange=$this->Prefix.imageChanged(); accept='image/x-png,image/gif,image/jpeg'>
                                </span>
                                <a href='#' class='btn btn-default fileinput-exists' data-dismiss='fileinput'>Remove</a>
                            </div>
