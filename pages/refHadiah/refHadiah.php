<?php
class refHadiah extends baseObject{
  var $Prefix = "refHadiah";
  var $formName = "refHadiahForm";
  var $tableName = "hadiah";
  var $username = "";
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($refHadiah_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($idSession)){
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($refHadiah_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$refHadiah_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/refHadiah/refHadiah.js'></script>
  ...
  function setKolomData($no,$arrayData){
    ...
    <tr class='$classRow'>
        ...
        <td style='vertical-align:middle;'>$namaHadiah</td>
                ...
                <div class='modal-body'>
                  <div class='form-group'>
                    <div class='row'>
                      ...
                      <div class='col-sm-9'>
                        ".$this->cmbQuery("idSession","","select id,judul from sesion","class='form-control'","-- SESSION --")."
                      </div>
                    </div>
                  </div>
                  <div class='form-group'>
                ...
                <div class='modal-body'>
                  <div class='form-group'>
                    <div class='row'>
                      <label class='col-sm-3 control-label' style='margin-top:6px;'>Session</label>
                      <div class='col-sm-9'>
                        ".$this->cmbQuery("idSession",$getDataEdit['id_session'],"select id,judul from sesion","class='form-control'","-- SESSION --")."
                      </div>
                    </div>
                  </div>
                  <div class='form-group'>
                          ...
                          <div class='fileinput-preview thumbnail' data-trigger='fileinput' style='width: 200px; height: 150px;'><img id='thumbnailImages' src ='".$getDataEdit['gambar']."'></img></div>
                            <div>
                                <span class='btn btn-default btn-file'>
                                    <span class='fileinput-image'>Select image</span>
                                    <span class='fileinput-exists'>Change</span>
                                    <input type='hidden' name='gambarHadiah' id='gambarHadiah' value='".$this->imageToBase($getDataEdit['gambar'])."'  >
                                    <input type='file' id='fileInputHadiah' onChange=$this->Prefix.imageChanged(); accept='image/x-png,image/gif,image/jpeg'>
                                </span>
                                <a href='#' class='btn btn-default fileinput-exists' data-dismiss='fileinput'>Remove</a>
                            </div>
