...
class bandingDatabaseLocal extends baseObject{
  ...
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'checkStruktur':{
        ...
        file_put_contents("/tmp/checkStruktur.txt",$jsonFileFilter);
        // $dataFileCheck = array(
        //   'isi' => $jsonFileFilter,
        ...
        $content = array("jumlahData" => sizeof($decodeJSON),"idFileCheck" => 1,
      ...
      case 'checkTriger':{
        ...
        $jsonFileFilter = json_encode($arrayFile);
        file_put_contents("/tmp/checkTriger.txt",$jsonFileFilter);
        // $dataFileCheck = array(
        //   'isi' => $jsonFileFilter,
        //   'username' => $this->username,
        //   'tanggal' => date("Y-m-d"),
      ...
      case 'checkRoutine':{
        ...
        $jsonFileFilter = json_encode($arrayFile);
        file_put_contents("/tmp/checkRoutine.txt",$jsonFileFilter);
        // $dataFileCheck = array(
        //   'isi' => $jsonFileFilter,
        //   'username' => $this->username,
        //   'tanggal' => date("Y-m-d"),
      ...
      case 'prosesCheckStruktur':{
        if($nomorUrut == ($jumlahData + 1)){
          ...
          $explodeListFile = json_decode(file_get_contents("/tmp/checkStruktur.txt"));
      ...
      case 'prosesCheckTriger':{
        if($nomorUrut == ($jumlahData + 1)){
          ...
          // $getDataFileCheck = $this->sqlArray($this->sqlQuery("select * from json_file_check where id ='$idFileCheck'"));
          $explodeListFile = json_decode(file_get_contents("/tmp/checkTriger.txt"));
      ...
      case 'prosesCheckRoutine':{
        if($nomorUrut == ($jumlahData + 1)){
          ...
          // $getDataFileCheck = $this->sqlArray($this->sqlQuery("select * from json_file_check where id ='$idFileCheck'"));
          $explodeListFile = json_decode(file_get_contents("/tmp/checkRoutine.txt"));
  ...
  function listParameter($id,$databaseName,$routineName,$routineType) {
    ...
    $comand =  str_replace("} \n{","},{",$this->sshCommand($sshConnection,"mysql -u".$usernameDatabase." -p".$passwordDatabase." -D information_schema -s -e 'select PARAMETER_NAME,DTD_IDENTIFIER,ORDINAL_POSITION from PARAMETERS where SPECIFIC_SCHEMA = $stringDatabaseName and SPECIFIC_NAME  = $stringRoutineName;' | awk '$contentAWK'"));
    return "[".str_replace(" ","",$comand)."]";
  }
