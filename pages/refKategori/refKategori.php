<?php
class refKategori extends baseObject{
  var $Prefix = "refKategori";
  var $formName = "refKategoriForm";
  var $tableName = "kategori";
  var $username = "";
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($refKategori_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($namaKategori)){
          $err = "Isi Nama Kategori";
        }elseif(empty($deskripsiKategori)){
          $err = "Isi Deskripsi Kategori";
        }
        if(empty($err)){
          $dataInsert = array(
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($refKategori_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$refKategori_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/refKategori/refKategori.js'></script>
                  ...
                  <div class='form-group'>
                    <div class='row'>
                      ...
                      <div class='col-sm-9'>
                        ".$this->textBox(array(
                          "id" => 'namaKategori',
                          "value" => $getDataEdit['nama_kategori'],
                        ))."
                      </div>
                    </div>
                  </div>
