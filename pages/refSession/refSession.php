<?php
class refSession extends baseObject{
  var $Prefix = "refSession";
  var $formName = "refSessionForm";
  var $tableName = "sesion";
  var $username = "";
  function jsonGenerator(){
    ...
    foreach ($_POST as $key => $value) {
      ...
      case 'Edit':{
        $content = $this->Edit($refSession_cb[0]);
  		break;
  		}
      case 'saveNew':{
  			if(empty($judulSession)){
          $err = "Isi Judul Session";
        }elseif(empty($kurunTanggal)){
          $err = "Isi Periode Tanggal";
        }elseif(empty($deskripsiSession)){
          $err = "Isi Deskripsi Session";
        }
        if(empty($err)){
      ...
      case 'Hapus':{
        for ($i=0; $i < sizeof($refSession_cb) ; $i++) {
          $query = "delete from $this->tableName where id = '".$refSession_cb[$i]."'";
          $this->sqlQuery($query);
        }
        $cek = $query;
        break;
  ...
  function loadScript(){
    return "
    <script type='text/javascript' src='js/refSession/refSession.js'></script>
            ...
            <div class='modal-content'>
                ...
                <div class='modal-body'>
                  ...
                  <div class='form-group'>
                    <div class='row'>
                      <label class='col-sm-3 control-label' style='margin-top:6px;'>Judul</label>
                      <div class='col-sm-9'>
                        ".$this->textBox(array(
                          "id" => 'judulSession',
                        ))."
                          ...
                          <div class='fileinput-preview thumbnail' data-trigger='fileinput' style='width: 200px; height: 150px;'><img id='thumbnailImages'></img></div>
                            <div>
                                <span class='btn btn-default btn-file'>
                                    ...
                                    <input type='hidden' name='gambarSession' id='gambarSession' >
                                    <input type='file' id='fileInputSession' onChange=$this->Prefix.imageChanged(); accept='image/x-png,image/gif,image/jpeg'>
                                </span>
                          ...
                          <div class='fileinput-preview thumbnail' data-trigger='fileinput' style='width: 200px; height: 150px;'><img id='thumbnailImages' src ='".$getDataEdit['gambar']."'></img></div>
                            <div>
                                <span class='btn btn-default btn-file'>
                                    <span class='fileinput-image'>Select image</span>
                                    <span class='fileinput-exists'>Change</span>
                                    <input type='hidden' name='gambarSession' id='gambarSession' value='".$this->imageToBase($getDataEdit['gambar'])."'  >
                                    <input type='file' id='fileInputSession' onChange=$this->Prefix.imageChanged(); accept='image/x-png,image/gif,image/jpeg'>
                                </span>
                                <a href='#' class='btn btn-default fileinput-exists' data-dismiss='fileinput'>Remove</a>
                            </div>
