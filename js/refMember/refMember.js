var refMember = new baseObject2({
	prefix : 'refMember',
	url : 'pages.php?page=refMember',
	...
	refreshList: function(){
		$.ajax({
			...
			url: refMember.url+'&API=refreshList',
			success: function(data) {
				var resp = eval('(' + data + ')');
				if(resp.err==''){
					$("#refMemberForm").html(resp.content.tableContent);
				}else{
					refMember.errorAlert(resp.err);
				}
			}
		});
	},
	...
	Edit: function(){
		var errMsg = refMember.getJumlahChecked();
		urlEdit = this.url;
		if(errMsg == ''){
			$.ajax({
				type:'POST',
  ...
  if(errMsg == '' || errMsg=='Pilih hanya satu data'){
    swal({
        ...
        function(){
          $.ajax({
              type:'POST',
							data : $("#"+refMember.formName).serialize(),
				      url: refMember.url+'&API=Hapus',
              success: function(data) {
                var resp = eval('(' + data + ')');
                if(resp.err==''){
                  refMember.suksesAlert("Data Terhapus",refMember.homePage);
                }else{
                  refMember.errorAlert(resp.err);
                }
              }
	  ...
	  var me = this;
		swal({
	      ...
	      showLoaderOnConfirm: true
							...
							$.ajax({
								type:'POST',
								data : $("#"+refMember.formName+"_input").serialize(),
								url: refMember.url+'&API=saveNew',
									success: function(data) {
									var resp = eval('(' + data + ')');
										if(resp.err==''){
										  refMember.suksesAlert("Data Tersimpan",refMember.homePage);
										}else{
											refMember.errorAlert(resp.err);
										}
									}
		...
		swal({
	      ...
	      showLoaderOnConfirm: true
							...
							$.ajax({
								type:'POST',
								data : $("#"+refMember.formName+"_input").serialize(),
								url: refMember.url+'&API=saveEdit',
									success: function(data) {
									var resp = eval('(' + data + ')');
										if(resp.err==''){
										  refMember.suksesAlert("Data Tersimpan",refMember.homePage);
