var refKategori = new baseObject2({
  prefix: "refKategori",
  url: "pages.php?page=refKategori",
  formName: "refKategoriForm",
  idFileCheck: 0,
  optionPushFile: 0,
  ...
  refreshList: function() {
    $.ajax({
      ...
      url: refKategori.url + "&API=refreshList",
      success: function(data) {
        var resp = eval("(" + data + ")");
        if (resp.err == "") {
          $("#refKategoriForm").html(resp.content.tableContent);
        } else {
          refKategori.errorAlert(resp.err);
        }
      }
    });
  },
  ...
  Edit: function() {
    var errMsg = refKategori.getJumlahChecked();
    urlEdit = this.url;
    if (errMsg == "") {
      $.ajax({
        type: "POST",
    ...
    if (errMsg == "" || errMsg == "Pilih hanya satu data") {
      swal(
        ...
        function() {
          $.ajax({
            type: "POST",
            data: $("#" + refKategori.formName).serialize(),
            url: refKategori.url + "&API=Hapus",
            success: function(data) {
              var resp = eval("(" + data + ")");
              if (resp.err == "") {
                refKategori.suksesAlert("Data Terhapus", refKategori.homePage);
              } else {
                refKategori.errorAlert(resp.err);
              }
            }
  ...
  homePage: function() {
    window.location = refKategori.url;
  },
  ...
  saveNew: function() {
    ...
    swal(
      ...
      function() {
        $.ajax({
          type: "POST",
          data: $("#" + refKategori.formName + "_input").serialize(),
          url: refKategori.url + "&API=saveNew",
          success: function(data) {
            var resp = eval("(" + data + ")");
            if (resp.err == "") {
              refKategori.suksesAlert("Data Tersimpan", refKategori.homePage);
            } else {
              refKategori.errorAlert(resp.err);
            }
          }
    ...
    swal(
      ...
      function() {
        $.ajax({
          type: "POST",
          data: $("#" + refKategori.formName + "_input").serialize(),
          url: refKategori.url + "&API=saveEdit",
          success: function(data) {
            var resp = eval("(" + data + ")");
            if (resp.err == "") {
              refKategori.suksesAlert("Data Tersimpan", refKategori.homePage);
