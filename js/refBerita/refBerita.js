var refBerita = new baseObject2({
  prefix: "refBerita",
  url: "pages.php?page=refBerita",
  formName: "refBeritaForm",
  idFileCheck: 0,
  optionPushFile: 0,
  ...
  refreshList: function() {
    $.ajax({
      ...
      url: refBerita.url + "&API=refreshList",
      success: function(data) {
        var resp = eval("(" + data + ")");
        if (resp.err == "") {
          $("#refBeritaForm").html(resp.content.tableContent);
        } else {
          refBerita.errorAlert(resp.err);
        }
      }
    });
  },
  ...
  Edit: function() {
    var errMsg = refBerita.getJumlahChecked();
    urlEdit = this.url;
    if (errMsg == "") {
      $.ajax({
        type: "POST",
    ...
    if (errMsg == "" || errMsg == "Pilih hanya satu data") {
      swal(
        ...
        function() {
          $.ajax({
            type: "POST",
            data: $("#" + refBerita.formName).serialize(),
            url: refBerita.url + "&API=Hapus",
            success: function(data) {
              var resp = eval("(" + data + ")");
              if (resp.err == "") {
                refBerita.suksesAlert("Data Terhapus", refBerita.homePage);
              } else {
                refBerita.errorAlert(resp.err);
              }
            }
  ...
  saveNew: function() {
    ...
    swal(
      ...
      function() {
        $.ajax({
          type: "POST",
          data: $("#" + refBerita.formName + "_input").serialize(),
          url: refBerita.url + "&API=saveNew",
          success: function(data) {
            var resp = eval("(" + data + ")");
            if (resp.err == "") {
              refBerita.suksesAlert("Data Tersimpan", refBerita.homePage);
            } else {
              refBerita.errorAlert(resp.err);
            }
          }
    ...
    swal(
      ...
      function() {
        $.ajax({
          type: "POST",
          data: $("#" + refBerita.formName + "_input").serialize(),
          url: refBerita.url + "&API=saveEdit",
          success: function(data) {
            var resp = eval("(" + data + ")");
            if (resp.err == "") {
              refBerita.suksesAlert("Data Tersimpan", refBerita.homePage);
  ...
  imageChanged: function() {
    var me = this;
    var filesSelected = document.getElementById("fileInputBerita").files;
    if (filesSelected.length > 0) {
      var fileToLoad = filesSelected[0];
      var fileReader = new FileReader();
      fileReader.onload = function(fileLoadedEvent) {
        $("#thumbnailImages").attr("src", fileLoadedEvent.target.result);
        $("#gambarBerita").val(fileLoadedEvent.target.result);
      };
