var refAds = new baseObject2({
  prefix: "refAds",
  url: "pages.php?page=refAds",
  formName: "refAdsForm",
  idFileCheck: 0,
  optionPushFile: 0,
  ...
  refreshList: function() {
    $.ajax({
      ...
      url: refAds.url + "&API=refreshList",
      success: function(data) {
        var resp = eval("(" + data + ")");
        if (resp.err == "") {
          $("#refAdsForm").html(resp.content.tableContent);
        } else {
          refAds.errorAlert(resp.err);
        }
      }
    });
  },
  ...
  Edit: function() {
    var errMsg = refAds.getJumlahChecked();
    urlEdit = this.url;
    if (errMsg == "") {
      $.ajax({
        type: "POST",
    ...
    if (errMsg == "" || errMsg == "Pilih hanya satu data") {
      swal(
        ...
        function() {
          $.ajax({
            type: "POST",
            data: $("#" + refAds.formName).serialize(),
            url: refAds.url + "&API=Hapus",
            success: function(data) {
              var resp = eval("(" + data + ")");
              if (resp.err == "") {
                refAds.suksesAlert("Data Terhapus", refAds.homePage);
              } else {
                refAds.errorAlert(resp.err);
              }
            }
  ...
  saveNew: function() {
    ...
    swal(
      ...
      function() {
        $.ajax({
          type: "POST",
          data: $("#" + refAds.formName + "_input").serialize(),
          url: refAds.url + "&API=saveNew",
          success: function(data) {
            var resp = eval("(" + data + ")");
            if (resp.err == "") {
              refAds.suksesAlert("Data Tersimpan", refAds.homePage);
            } else {
              refAds.errorAlert(resp.err);
            }
          }
